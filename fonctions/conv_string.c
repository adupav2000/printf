...
/*   Updated: 2020/01/02 09:21:34 by adu-pavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *conv_string_meth_1(char *str, void *content)
{
    ...
    if (min <= max)
    {
        if (ft_strlen((char *)content) > min)
        {
            if (!(ret_val = ft_strnew(ft_strlen((char *)content) + 1)))
                return (0);
            ft_strlcpy(ret_val,
                (char *)content,
    ...
    if (min > max)
    {
        ...
        {
            if (!(ret_val = 
            ft_strnew(min + (min - ft_strlen((char *)content) + 1))))
                return (0);
            while (i <= min)
                ret_val[i++] = ' ';
            ft_strlcat(ret_val, (char *)content,
                (min - ft_strlen((char *)content) + 1));
            return (ret_val);
        }
    }
    return (0);
...
char *conv_string_meth_3(char *str, void *content)
{
    ...
    if (-min < max && ft_strlen((char *)content) < -min)
    {
        i = ft_strlen((char *)content);
        if (!(ret_val = ft_strnew(-min)))
            return (0);
...
char *conv_string_meth_4(char *str, void *content, int opt)
{
    ...
    if (opt == 1)
    {
        if (!(ret_val = ft_strnew(ft_strlen((char *)content))))
        return (0);
        strlcpy(ret_val, (char *)content, ft_strlen((char *)content) + 1);
    }
    else if (opt == 2)
    {
        max = ft_get_lim_string_max(str);
...
char *conv_string(char *str, void *content)
{
    ...
    if (!(content))
    {
        if (!(content = ft_strnew(ft_strlen("(null)"))))
            return (0);
        ft_strlcpy(content, "(null)", ft_strlen("(null)"));
    }
